...
if (!empty($_POST)) {
	...
	if (!empty($name)) {
		if ($name == '') {
			$sql = 'insert into category(cat_name, description) values ("'.$name.'", "'.$description.'")';
		}else{
			$sql = 'update category set cat_name = "'.$name.'", description = "'.$description.'" where cat_name = '.$name;
		}
		//Luu vao database
		execute($sql);
			...
			<div class="panel-body">
				<form method="post">
					<div class="form-group">
						...
						<input required="true" type="text" class="form-control" id="cat_name" name="cat_name" value="<?=$name?>">
					</div>
					<div class="form-group">
